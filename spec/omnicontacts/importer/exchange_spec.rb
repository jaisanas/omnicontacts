require "spec_helper"
require "omnicontacts/importer/exchange"

describe OmniContacts::Importer::Exchange do

  let(:permissions) { "perm1, perm2" }
  let(:exchange) { OmniContacts::Importer::Exchange.new({}, "client_id", "client_secret", {:permissions => permissions}) }
  ...
  describe "fetch_contacts_using_access_token" do
    ...
    before(:each) do
      exchange.instance_variable_set(:@env, {"HTTP_HOST" => "http://example.com"})
    end

    it "should request the contacts by providing the token in the url" do
      exchange.should_receive(:https_get) do |host, path, params, headers|
      ...
      exchange.fetch_contacts_using_access_token token, token_type
    end

    it "should set requested permissions in the authorization url" do
      exchange.authorization_url.should match(/scope=#{Regexp.quote(CGI.escape(permissions))}/)
    end

    it "should correctly parse id, name, email, gender, birthday, profile picture, relation and email hashes" do
      exchange.should_receive(:https_get).and_return(self_response)
      exchange.should_receive(:https_get).and_return(contacts_as_json)
      result = exchange.fetch_contacts_using_access_token token, token_type
    ...
    it "should correctly parse and set logged in user information" do
      ...
      user = exchange.instance_variable_get(:@env)["omnicontacts.user"]
